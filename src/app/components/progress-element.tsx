...
export const ProgressElement: React.FC<TProps> = ({ step, idx, activeIndex, progress, onClick }) => {
  ...
  return isActive ? (
    ...
    >
      ...
      <div className="text-sm mb-4 transition-opacity duration-300 opacity-100">{step.description}</div>
    </div>
  ) : (
    <div
      key={step.title}
      className={`text-gray-elements transition-all duration-300 overflow-hidden relative`}
      style={{ minHeight: '88px' }}
      onClick={() => !isActive && onClick?.(idx)}
      tabIndex={0}
      role="button"
